...
def detect_faces(our_image):
  ...
  for (x, y, w, h) in faces:
    ...
    cv2.putText(img, f'Personne {nb_personnes}', (x, y - int(hauteur_bandeau/3)), cv2.FONT_HERSHEY_SIMPLEX, .5, color_text)
  return img,faces 
...
def detect_eyes(our_image):
  ...
  for (ex,ey,ew,eh) in eyes:
    cv2.rectangle(img,(ex,ey),(ex+ew,ey+eh),color_cadre,2)
  return img
...
def detect_smiles(our_image):
  ...
  for (x, y, w, h) in smiles:
    cv2.rectangle(img, (x, y), (x+w, y+h), color_cadre, 2)
  return img
